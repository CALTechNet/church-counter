...
Primary: OpenCV Stitcher (RANSAC-based feature matching + multi-band blending)
...
# Also export as enhance_frame for any callers using that name
enhance_frame = _enhance_for_stitching
...
def stitch_frames(
    ...
    via OpenCV's built-in Stitcher (multi-band blending, no ghosting).

    grid_shape is accepted for API compatibility but not used — the OpenCV
    Stitcher discovers overlap relationships via feature matching.
    ...
    # Try stitching with enhanced frames (better keypoint detection in low light)
    ...
    status2, pano2 = stitcher2.stitch(unique)
